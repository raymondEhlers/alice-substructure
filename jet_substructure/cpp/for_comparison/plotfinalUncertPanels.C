...
void plotfinalUncertPanels()
{
  ...
  if (mine == 1) {
    ...
    test[9] = new TFile("UnfoldKgzg0.2ppDefaultRmax025.root");
  ...
  if (mine == 2) {
    ...
    test[9] = new TFile("UnfoldKgzg0.4ppDefaultRmax025dyn.root");
  ...
  if (mine == 3) {
    ...
    test[9] = new TFile("UnfoldKgzg0.4ppDefaultRmax025Time.root");
  ...
  if (mine == 4) {
    ...
    test[9] = new TFile("UnfoldKgzg0.4ppDefaultRmax025nocut.root");
  ...
  for (Int_t k = 0; k < 12; k++) {
    ...
    if (k == 2)
      itera = (TH2D*)test[k]->Get(Form("Bayesian_Unfoldediter%d.root", 4));
    if (k == 3)
      itera = (TH2D*)test[k]->Get(Form("Bayesian_Unfoldediter%d.root", 7));
  ...
  for (Int_t j = 0; j < shape[0]->GetNbinsX(); j++) {
    ...
    if (erryprior >= 0) {
      erryaprior[j] = erryprior;
      errybprior[j] = 0;
    }
    if (erryprior < 0) {
      errybprior[j] = -1 * erryprior;
      erryaprior[j] = 0;
    }
    ...
    if (errybin < 0) {
      errybbin[j] = -1 * errybin;
      erryabin[j] = 0;
    }

    if (erryiter > 0 && erryitermenos > 0) {
      erryaiter[j] = TMath::Max(erryiter, erryitermenos);
      errybiter[j] = 0;
    }
    if (erryiter < 0 && erryitermenos < 0) {
      erryaiter[j] = 0;
    ...
    if (erryiter > 0 && erryitermenos < 0) {
      erryaiter[j] = erryiter;
      errybiter[j] = -1 * erryitermenos;
    }
    if (erryiter < 0 && erryitermenos > 0) {
      erryaiter[j] = erryitermenos;
      errybiter[j] = -1 * erryiter;
    }

    if (errytrunc > 0 && errytruncmenos > 0) {
      erryatrunc[j] = TMath::Max(errytrunc, errytruncmenos);
      errybtrunc[j] = 0;
    }
    if (errytrunc < 0 && errytruncmenos < 0) {
      erryatrunc[j] = 0;
    ...
    if (errytrunc > 0 && errytruncmenos < 0) {
      erryatrunc[j] = errytrunc;
      errybtrunc[j] = -1 * errytruncmenos;
    }
    if (errytrunc < 0 && errytruncmenos > 0) {
      erryatrunc[j] = errytruncmenos;
      errybtrunc[j] = -1 * errytrunc;
    }

    if (errydisplace > 0 && errydisplace2 > 0) {
      erryadisplace[j] = TMath::Max(errydisplace, errydisplace2);
      errybdisplace[j] = 0;
    }
    if (errydisplace < 0 && errydisplace2 < 0) {
      erryadisplace[j] = 0;
    ...
    if (errydisplace > 0 && errydisplace2 < 0) {
      erryadisplace[j] = errydisplace;
      errybdisplace[j] = -1 * errydisplace2;
    }
    if (errydisplace < 0 && errydisplace2 > 0) {
      erryadisplace[j] = errydisplace2;
      errybdisplace[j] = -1 * errydisplace;
    }

    if (erryeffi >= 0) {
      erryaeffi[j] = erryeffi;
      errybeffi[j] = erryeffi;
    }
    if (erryeffi < 0) {
      errybeffi[j] = -1 * erryeffi;
      erryaeffi[j] = -1 * erryeffi;
    }

    erruncorrup[j] =
     TMath::Sqrt(erryaiter[j] * erryaiter[j] + erryatrunc[j] * erryatrunc[j] + erryaeffi[j] * erryaeffi[j] +
           erryadisplace[j] * erryadisplace[j] + erryabin[j] * erryabin[j] + erryaprior[j] * erryaprior[j]);
    erruncorrdo[j] =
     TMath::Sqrt(errybiter[j] * errybiter[j] + errybtrunc[j] * errybtrunc[j] + errybeffi[j] * errybeffi[j] +
           errybdisplace[j] * errybdisplace[j] + errybbin[j] * errybbin[j] + errybprior[j] * errybprior[j]);
  ...
  shapeuncorr[0] =
   new TGraphAsymmErrors(shape[0]->GetNbinsX(), vectorx, vectory, errxa, errxb, erruncorrdo, erruncorrup);
  ...
  shapeuncorr2[0] =
   new TGraphAsymmErrors(shape[0]->GetNbinsX(), vectorx, relative, errxa, errxb, erruncorrdo2, erruncorrup2);
  ...
  shapeuncorr3[0] =
   new TGraphAsymmErrors(shape[0]->GetNbinsX(), vectorx, relative, errxa, errxb, erruncorrdo3, erruncorrup3);
  ...
  shapeuncorr4[0] =
   new TGraphAsymmErrors(shape[0]->GetNbinsX(), vectorx, relative, errxa, errxb, erruncorrdo4, erruncorrup4);
  ...
  shapeuncorr5[0] =
   new TGraphAsymmErrors(shape[0]->GetNbinsX(), vectorx, relative, errxa, errxb, erruncorrdo5, erruncorrup5);
  ...
  shapeuncorr6[0] =
   new TGraphAsymmErrors(shape[0]->GetNbinsX(), vectorx, relative, errxa, errxb, erruncorrdo6, erruncorrup6);
  ...
  shapeuncorr8[0] =
   new TGraphAsymmErrors(shape[0]->GetNbinsX(), vectorx, relative, errxa, errxb, erruncorrdo8, erruncorrup8);
  ...
  canv2->SaveAs("rgunfolded.pdf");

  if (mine < 2)
    cout << "integral" << shape[0]->Integral(1, 8) << " "
       << 1 - shape[0]->Integral(1, 1) / shape[0]->Integral(1, -1) << " " << shape[0]->GetBinContent(1) << " "
       << shape[0]->GetBinError(1) << " " << shape[0]->GetBinCenter(1) << endl;
  cout << "true integral" << htrue1->Integral(1, 8) << " " << 1 - htrue1->Integral(1, 1) / htrue1->Integral(1, -1)
     << " " << htrue1->GetBinContent(1) << " " << htrue1->GetBinError(1) << " " << htrue1->GetBinCenter(1) << endl;
  ...
  shapeuncorr2[0]->GetXaxis()->SetTitle("k_{T} (GeV/#it{c})");
  shapeuncorr2[0]->GetYaxis()->SetTitle("1/N_{jets} dN/dk_{T} (GeV/#it{c})^{-1}");
  ...
  DrawLatex(0.23, 0.45, 1, "pp #sqrt{#it{s}} = 5.02 TeV", 0.03);
  DrawLatex(0.23, 0.4, 1, Form("Anti-#it{k}_{T}  charged jets, #it{R} = 0.4, SD zcut = 0.4"), 0.03);
  // DrawLatex(0.22,0.88,1,"ALICE Preliminary",0.05);
  DrawLatex(0.23, 0.35, 1, "60 < p_{T}^{jet,ch} < 80 GeV/#it{c}", 0.03);
  if (mine == 0) {
    DrawLatex(0.23, 0.3, 1, "z > 0.4 Leading #it{k}_{T}", 0.03);
    canv3->SaveAs("uncert_zcut4.pdf");
  }
  if (mine == 1) {
    DrawLatex(0.23, 0.3, 1, "z > 0.2 Leading #it{k}_{T}", 0.03);
    canv3->SaveAs("uncert_zcut2.pdf");
  }
  if (mine == 2) {
    DrawLatex(0.23, 0.3, 1, "Dynamical #it{k}_{T}", 0.03);
    canv3->SaveAs("uncert_dyn.pdf");
  }
  if (mine == 3) {
    DrawLatex(0.23, 0.3, 1, "Dynamical #it{t}_{f}", 0.03);
    canv3->SaveAs("uncert_dyntime.pdf");
  }
  if (mine == 4) {
    DrawLatex(0.23, 0.3, 1, "Leading #it{k}_{T}", 0.03);
    canv3->SaveAs("uncert_nocut.pdf");
  }
  ...
  // p1->Draw();
  TLegend* legi = new TLegend(0.2, 0.42, 0.3, 0.52);
  ...
  if (mine == 0)
    shape[0]->GetYaxis()->SetTitle("(1/N_{jet}) dN/d#it{k}_{T} (GeV/#it{c})^{-1}");
  if (mine == 1)
    shape[0]->GetYaxis()->SetTitle("(1/N_{jet}) dN/d#it{k}_{T} (GeV/#it{c})^{-1}");
  if (mine == 2)
    shape[0]->GetYaxis()->SetTitle("(1/N_{jet}) dN/d#it{k}_{T} (GeV/#it{c})^{-1}");
  ...
  shape[0]->GetYaxis()->SetRangeUser(7e-3, 0.8);
  shape[0]->Draw("pe");
  ...
  if (mine == 0)
    lat->DrawLatex(0.93, 0.73, "z > 0.4 Leading #it{k}_{T}");
  if (mine == 1)
    lat->DrawLatex(0.93, 0.73, "z > 0.2 Leading #it{k}_{T}");
  if (mine == 2)
    lat->DrawLatex(0.93, 0.73, "Dynamical #it{k}_{T}");
  if (mine == 3)
    lat->DrawLatex(0.93, 0.73, "Dynamical #it{t}_{f}");
  if (mine == 4)
    lat->DrawLatex(0.93, 0.73, "Leading #it{k}_{T}");
  if (mine == 0)
    lat->DrawLatex(0.93, 0.63, "f_{tagged}=0.35");
  if (mine == 1)
    lat->DrawLatex(0.93, 0.63, "f_{tagged}=0.84");
  ...
  if (mine == 0)
    h1_ratio->GetXaxis()->SetTitle("#it{k}_{T} (GeV/#it{c})");
  if (mine == 1)
    h1_ratio->GetXaxis()->SetTitle("#it{k}_{T} (GeV/#it{c})");
  if (mine == 2)
    h1_ratio->GetXaxis()->SetTitle("#it{k}_{T} (GeV/#it{c})");
  ...
  h1_ratio->SetTitle("");
  h1_ratio->GetYaxis()->SetRangeUser(0.6, 1.65);
  h1_ratio->GetXaxis()->SetLabelOffset(0.001);
  ...
  if (mine == 0)
    canv4->SaveAs("ktunfolded_zcut4.pdf");
  if (mine == 1)
    canv4->SaveAs("ktunfolded_zcut2.pdf");
  if (mine == 2)
    canv4->SaveAs("ktunfolded_dyn.pdf");
  if (mine == 3)
    canv4->SaveAs("ktunfolded_dyntime.pdf");
  if (mine == 4)
    canv4->SaveAs("ktunfolded_nocut.pdf");
  ...
  TString cOutFile;
  if (mine == 0)
    cOutFile = Form("result_leadingktzcut04.root");
  if (mine == 1)
    cOutFile = Form("result_leadingktzcut02.root");
  if (mine == 2)
    cOutFile = Form("result_dynamickt.root");
  if (mine == 3)
    cOutFile = Form("result_dynamictf.root");
  if (mine == 4)
    cOutFile = Form("result_leadingktnocut.root");
  TFile* fOut = 0;
