...
void RooSimpleRgPbPbRmax025ClosureWeight1_var(
 TString cFiles2 =
  "/Users/leticiacunqueiro/2019RawData/SmearedReferenceSemiEventWiseHard0.4Rmax025Matching/files1.txt")
{
  ...
  while (infile2 >> filename2) {
    ...
    for (int iEntry = 0; iEntry < nEv; iEntry++) {
      ...
      if (rnd > 0.9) {
        h2smearedb->Fill(rg, ptJet, scalefactor * myw);
        h2trueb->Fill(rgMatch, ptJetMatch, scalefactor * myw);
        continue;
      }
  ...
  TFile* fout = new TFile(Form("UnfoldRgzg0.4EventWiseDefaultRmax025ClosureWeight1_var.root"), "RECREATE");
  fout->cd();
  effok->Write();
  effok3->Write();
  effok5->Write();
...
int main()
{
  RooSimpleRgPbPbRmax025ClosureWeight1_var();
  return 0;
