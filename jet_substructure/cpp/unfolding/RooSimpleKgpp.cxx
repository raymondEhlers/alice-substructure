...
void RooSimpleKgpp(TString cFiles2="/Users/leticiacunqueiro/2019RawData/RaymondLeadKt/pythia/2110/skim/files.txt")
{
#ifdef __CINT__
  ...
  TTree *data=(TTree*)inputdata->Get("tree"); 
   ...
   h2smearednocuts=new TH2D("smearednocuts","smearednocuts",7,xbinsc,8,0,160);
   //true correlations with measured cuts
    TH2D *h2true(0);
    h2true=new TH2D("true","true",7,xbinsc,8,0,160);
    //full true correlation
    TH2D *h2fulleff(0);
    h2fulleff=new TH2D("truef","truef",7,xbinsc,8,0,160);
  ...
  data->SetBranchAddress("leading_kt_z_cut_04_data_kt", &leading_kt_z_cut_04_kt); 
   ...
   if(jet_pt_data>85 || jet_pt_data<20) continue;
   if(leading_kt_z_cut_04_kt>8 || leading_kt_z_cut_04_kt<0.25) continue;
  ...
  int count=0;
    while(infile2>>filename2){
      ...
      for(int iEntry=0; iEntry< nEv; iEntry++){
       ...
       if(jet_pt_data>85 || jet_pt_data<20) continue;
          if(leading_kt_z_cut_04_data_kt>8 ||leading_kt_z_cut_04_kt<0.25) continue;
	  ...
	  h2smeared->Fill(leading_kt_z_cut_04_data_kt,jet_pt_data,scale_factor);
      	...
      	response.Fill(leading_kt_z_cut_04_data_kt,jet_pt_data,leading_kt_z_cut_04_matched_kt,jet_pt_matched,scale_factor);
 ...
 TFile *fout=new TFile (Form("UnfoldKgzg0.4ppDefaultRmax025.root"),"RECREATE");
 fout->cd();
 effok->Write(); 
 effok3->Write();
 effok5->Write();
...
#ifndef __CINT__
int main () { RooSimpleKgpp(); return 0; }  // Main program when run stand-alone
#endif
