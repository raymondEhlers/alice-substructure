...
void RooSimpleKgppzcut4(Int_t kindex = 0)
{
  ...
  if (kindex == 2)
    cFiles2 = "/Users/leticiacunqueiro/2019RawData/RaymondLeadKt/pythia/2161_Eff/skim/files.txt";
  ifstream infile2;
  infile2.open(cFiles2.Data());
  char filename2[300];

  TFile* RatioFile =
   new TFile("/Users/leticiacunqueiro/2019RawData/RaymondLeadKt/pythia/2110/skim/fileRatioDataMCppzcut4.root");
  TH2D* historatio;
  ...
  for (int iEntry = 0; iEntry < nEv; iEntry++) {
    ...
    if (jet_pt_data > xbins[5] || jet_pt_data < xbins[0])
      continue;
    if (kindex == 6) {
      if (leading_kt_z_cut_04_kt > 8)
        continue;
      if (leading_kt_z_cut_04_kt < 0.25 && leading_kt_z_cut_04_kt >= 0)
        continue;
      if (leading_kt_z_cut_04_kt < 0)
        leading_kt_z_cut_04_kt = 0.12;
    }
  ...
  while (infile2 >> filename2) {
    ...
    for (int iEntry = 0; iEntry < nEv; iEntry++) {
      ...
      if (jet_pt_matched > 160)
        continue;

      if (leading_kt_z_cut_04_matched_kt > 15)
        continue;
      if (leading_kt_z_cut_04_matched_kt < 0)
        leading_kt_z_cut_04_matched_kt = -0.25;
      ...
      responsenotrunc.Fill(leading_kt_z_cut_04_data_kt, jet_pt_data, leading_kt_z_cut_04_matched_kt,
                 jet_pt_matched, scale_factor);

      if (jet_pt_data > xbins[5] || jet_pt_data < xbins[0])
        continue;

      if (kindex == 6) {
        if (leading_kt_z_cut_04_data_kt > 8)
          continue;
        if (leading_kt_z_cut_04_data_kt < 0.25 && leading_kt_z_cut_04_data_kt >= 0)
          continue;
        if (leading_kt_z_cut_04_data_kt < 0)
          leading_kt_z_cut_04_data_kt = 0.12;
      }
      ...
      if (kindex != 6) {
        ...
        if (leading_kt_z_cut_04_data_kt < 0.25 && leading_kt_z_cut_04_data_kt >= 0)
          continue;
        if (leading_kt_z_cut_04_data_kt < 0)
          leading_kt_z_cut_04_data_kt = 8.5;
      }
      ...
      if (kindex == 5) {
        if (jet_pt_matched < 20) {
          if (leading_kt_z_cut_04_matched_kt < 0.)
            myw = historatio->GetBinContent(8, 1);
          if (leading_kt_z_cut_04_matched_kt >= 0. && leading_kt_z_cut_04_matched_kt < 0.5)
            myw = historatio->GetBinContent(1, 1);
          if (leading_kt_z_cut_04_matched_kt >= 0.5 && leading_kt_z_cut_04_matched_kt < 1)
            myw = historatio->GetBinContent(2, 1);
          if (leading_kt_z_cut_04_matched_kt >= 1 && leading_kt_z_cut_04_matched_kt < 2)
            myw = historatio->GetBinContent(3, 1);
          if (leading_kt_z_cut_04_matched_kt >= 2 && leading_kt_z_cut_04_matched_kt < 4)
            myw = historatio->GetBinContent(5, 1);
          if (leading_kt_z_cut_04_matched_kt >= 4 && leading_kt_z_cut_04_matched_kt < 6)
            myw = historatio->GetBinContent(6, 1);
          if (leading_kt_z_cut_04_matched_kt >= 6 && leading_kt_z_cut_04_matched_kt < 8)
            myw = historatio->GetBinContent(7, 1);
          if (leading_kt_z_cut_04_matched_kt >= 8)
            myw = historatio->GetBinContent(7, 1);
        }
        ...
        if (jet_pt_matched >= 30 && jet_pt_matched < 40) {
          if (leading_kt_z_cut_04_matched_kt < 0.)
            myw = historatio->GetBinContent(8, 2);
          if (leading_kt_z_cut_04_matched_kt >= 0. && leading_kt_z_cut_04_matched_kt < 0.5)
        ...
        if (jet_pt_matched >= 40 && jet_pt_matched < 50) {
          if (leading_kt_z_cut_04_matched_kt < 0.)
            myw = historatio->GetBinContent(8, 3);
          if (leading_kt_z_cut_04_matched_kt >= 0. && leading_kt_z_cut_04_matched_kt < 0.5)
        ...
        if (jet_pt_matched >= 50 && jet_pt_matched < 60) {
          if (leading_kt_z_cut_04_matched_kt < 0.)
            myw = historatio->GetBinContent(8, 4);
          if (leading_kt_z_cut_04_matched_kt >= 0. && leading_kt_z_cut_04_matched_kt < 0.5)
        ...
        if (jet_pt_matched >= 60 && jet_pt_matched < 85) {
          if (leading_kt_z_cut_04_matched_kt < 0.)
            myw = historatio->GetBinContent(8, 5);
          if (leading_kt_z_cut_04_matched_kt >= 0. && leading_kt_z_cut_04_matched_kt < 0.5)
      ...
      response.Fill(leading_kt_z_cut_04_data_kt, jet_pt_data, leading_kt_z_cut_04_matched_kt, jet_pt_matched,
             scale_factor * myw);
    }
    count = count + 1;
  ...
  TFile* fout;
  if (kindex == 0)
    fout = new TFile(Form("UnfoldKgzg0.4ppDefaultRmax025.root"), "RECREATE");
  if (kindex == 1)
    fout = new TFile(Form("UnfoldKgzg0.4ppDefaultRmax025Bin.root"), "RECREATE");
  if (kindex == 2)
    fout = new TFile(Form("UnfoldKgzg0.4ppDefaultRmax025Eff.root"), "RECREATE");
  if (kindex == 3)
    fout = new TFile(Form("UnfoldKgzg0.4ppDefaultRmax025TruncLow.root"), "RECREATE");
  if (kindex == 4)
    fout = new TFile(Form("UnfoldKgzg0.4ppDefaultRmax025TruncHigh.root"), "RECREATE");
  if (kindex == 5)
    fout = new TFile(Form("UnfoldKgzg0.4ppDefaultRmax025Prior.root"), "RECREATE");
  if (kindex == 6)
    fout = new TFile(Form("UnfoldKgzg0.4ppDefaultRmax025Displaced.root"), "RECREATE");
  fout->cd();
  effok->Write();
  effok3->Write();
  effok5->Write();
  ...
  for (int jar = 1; jar < 16; jar++) {
    ...
    // 	    TH2D *hCorr= (TH2D*) CorrelationHistShape(covmat, Form("corr%d",k), "Covariance
    // matrix",h2true->GetNbinsX(),h2true->GetNbinsY(),k);
    ...
    // 	    TH2D *hCorr= (TH2D*) CorrelationHistPt(covmat, Form("corr%d",k), "Covariance
    // matrix",h2true->GetNbinsX(),h2true->GetNbinsY(),k);
