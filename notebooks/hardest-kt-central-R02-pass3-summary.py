...
#
# Summarize plots for central PbPb R = 0.2, pass 3

# Table of Contents:
#
# 0. [Subjet matching](#Subjet-matching)
...
#text_label += "\n" + pb.label_to_display_string["collision_system"][collision_system].format(main_system=r"0-10\%")
#text_label += "\n" + f"${jet_pt_bin.display_str(label=prefix)}$"
#tag = jet_pt_bin.histogram_str(prefix)
system_label = pb.label_to_display_string["collision_system"][collision_system].format(main_system=r"0-10\%")
...
# Output
_output_dir = output_dir / collision_system / "RDF" / "jupyter" / "2023-02-HP" / "central_R02" / "pass3"
...
for dataset_name in ["LHC20g4_embedded_into_LHC18qr_central_R02_0071"]:
    for grooming_method in nb_utils.all_grooming_methods:
        try:
            hists_embedding.update(
                nb_utils.load_histograms(
...
tag = jet_pt_bin.histogram_str(prefix)
system_label = pb.label_to_display_string["collision_system"][collision_system]
...
# Output
_output_dir = output_dir / collision_system / "RDF" / "jupyter" / "2023-02-HP" /"central_R02" / "pass3" / prefix
_output_dir.mkdir(parents=True, exist_ok=True)
_fig_output_dir = _output_dir / "png"
